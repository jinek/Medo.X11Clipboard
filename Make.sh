...
FOUND_PROJECTS=0
for PROJECT_FILE in $(find $BASE_DIRECTORY/src/_MultiFramework -name "*.csproj" | sort); do
    FOUND_PROJECTS=1
    break
done
...
PACKAGE_FRAMEWORKS=`cat "$PROJECT_FILE" | grep "<TargetFrameworks" | sed 's^</\?TargetFrameworks\?>^^g' | tr ';' ' ' | xargs`
...
function debug() {
    ...
    for PROJECT_FILE in $(find $BASE_DIRECTORY/src/_MultiFramework -name "*.csproj" | sort); do
        ...
        rm -r $BASE_DIRECTORY/src/_MultiFramework/bin 2>/dev/null
        dotnet build "$PROJECT_FILE" \
                    --configuration "Debug" \
                    --verbosity "minimal" \
                    || return 1
        for FRAMEWORK in $PACKAGE_FRAMEWORKS; do
            cp -r "$BASE_DIRECTORY/src/_MultiFramework/bin/Debug/$FRAMEWORK/" "$BASE_DIRECTORY/bin/$BASE_NAME/" 2>/dev/null
...
function release() {
    ...
    for PROJECT_FILE in $(find $BASE_DIRECTORY/src/_MultiFramework -name "*.csproj" | sort); do
        ...
        dotnet build "$PROJECT_FILE" \
                    ...
                    || return 1
        for FRAMEWORK in $PACKAGE_FRAMEWORKS; do
            cp -r "$BASE_DIRECTORY/src/_MultiFramework/bin/Release/$FRAMEWORK/" "$BASE_DIRECTORY/bin/$BASE_NAME" 2>/dev/null
            if [[ $? -ne 0 ]]; then return 1; fi
        done
    done
...
function test() {
    echo
    FOUND_PROJECTS=0
    for TEST_PROJECT_FILE in $(find $BASE_DIRECTORY/tests/_MultiFramework -name "*.csproj" | sort); do
